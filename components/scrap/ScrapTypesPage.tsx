...
export default function ScrapTypesPage({ can, onNotice }: { can: (p: string) => boolean; onNotice: (m: string) => void }) {
  ...
  useEffect(() => {
    let mounted = true;
    supabase.from('scrap_items').select('*').order('name').then(({ data }) => {
      if (!mounted) return;
      setItems(((data ?? []) as ScrapItem[]).sort(byScrapTypeOrder));
      setLoading(false);
    });
  ...
  return (
    <div>
      <div className="page-heading">
        <div><p className="eyebrow">Scrap yard</p><h1>Scrap Types &amp; Rates</h1><p className="muted">Manage the scrap types Joseph buys and today&apos;s rate per kg.</p></div>
      </div>
