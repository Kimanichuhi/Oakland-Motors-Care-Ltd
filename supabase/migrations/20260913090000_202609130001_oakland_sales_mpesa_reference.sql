/*
# Oakland Motor Care Ltd — capture the M-Pesa code and transaction time on a sale

The sales module already asks for a payment method (CASH/MPESA/BANK/CARD/CREDIT/
JOB_CARD/OTHER) but never captured any reference for it — for MPESA specifically,
staff need to record the M-Pesa transaction code and the time the money was sent
so a sale can be reconciled against the phone/SMS confirmation later.

Reuses the existing `payments.reference` naming convention rather than inventing a
new one. `payment_reference_at` is left nullable/unconstrained for non-MPESA sales
(a bank transfer or card slip could still carry a reference without a "sent at"
time), but MPESA sales are required to have both.
*/

alter table public.sales add column if not exists payment_reference text;
alter table public.sales add column if not exists payment_reference_at timestamptz;
