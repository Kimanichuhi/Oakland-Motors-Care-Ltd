...
describe('generate_job_card_number', () => {
  ...
  it('never produces duplicate numbers under concurrent calls', async () => {
    const { client: admin } = await createStaffUser('ADMIN', 'admin-concurrent');
    const results = await Promise.all(Array.from({ length: 10 }, () => admin.rpc('generate_job_card_number')));
  ...
  it('rejects a role without job.create', async () => {
    const { client: manager } = await createStaffUser('MANAGER', 'manager-numbering');
    const { error } = await manager.rpc('generate_job_card_number');
